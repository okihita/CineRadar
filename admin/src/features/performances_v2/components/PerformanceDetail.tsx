...
export function PerformanceDetail({ movieId }: PerformanceDetailProps) {
    ...
    useEffect(() => {
        async function fetchMovie() {
            try {
                const res = await fetch(`/api/performance_v2/${movieId}`);
    ...
    useEffect(() => {
        async function fetchHistory() {
            try {
                const res = await fetch(`/api/performance_v2/${movieId}/history`);
                const data = await res.json();
                if (data.success) {
    ...
    if (!movie) {
        return (
            <div className="flex flex-col items-center justify-center min-h-[50vh] gap-4">
                ...
                <Button onClick={() => router.push('/performances_v2')}>Back to Performances</Button>
            </div>
        );
    }
