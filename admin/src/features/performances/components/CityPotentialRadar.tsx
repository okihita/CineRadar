          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b bg-muted/10 text-left text-muted-foreground text-xs uppercase tracking-wider">
                ...
                >
                  City <SortIcon field="city" sortField={sortField} />
                </th>
                <th
                ...
                >
                  <div className="flex items-center justify-end gap-1">
                    ...
                    <span>Theatres</span>
                    <SortIcon field="theatres" sortField={sortField} />
                  </div>
                </th>
                <th
                  className="py-2.5 px-4 font-medium text-right cursor-pointer hover:bg-muted/50"
                ...
                >
                  Shows <SortIcon field="shows" sortField={sortField} />
                </th>
                <th
                  className="py-2.5 px-4 font-medium text-right cursor-pointer hover:bg-muted/50"
                  onClick={() => handleSort("potential")}
                >
                  Potential <SortIcon field="potential" sortField={sortField} />
                </th>
                <th
                  className="py-2.5 px-4 font-medium w-32 cursor-pointer hover:bg-muted/50"
                  onClick={() => handleSort("occupancy")}
                >
                  Occupancy <SortIcon field="occupancy" sortField={sortField} />
                </th>
                <th
                  className="py-2.5 px-4 font-medium text-right cursor-pointer hover:bg-muted/50"
                  onClick={() => handleSort("sold")}
                >
                  Sold <SortIcon field="sold" sortField={sortField} />
                </th>
              </tr>
            </thead>
            <tbody>
                    ...
                    <div className="flex items-center gap-2">
                      <div className="flex-1 h-1.5 bg-muted rounded-full overflow-hidden">
                        <div
                          ...
                          style={{
                            width: `${Math.min(city.occupancyPct, 100)}%`,
                          }}
                        />
                      </div>
