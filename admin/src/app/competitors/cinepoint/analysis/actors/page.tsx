...
import {
  Loader2, Star, ArrowLeft, ArrowUpRight, Film,
  Users, TrendingUp, Search, ChevronRight, Eye,
} from 'lucide-react';
import { cn } from '@/lib/utils';
import { Badge } from '@/components/ui/badge';
import { Card, CardContent, CardHeader, CardTitle } from '@/components/ui/card';

// ─── Types (shared with analysis page) ─────────────────────
        ...
        <div className="px-6 py-8 space-y-6 max-w-[1400px] mx-auto">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-[10px] text-muted-foreground/60">Loading actor performance data…</p>
            </div>
          </div>
    ...
    <div className="px-6 py-8 space-y-6 max-w-full mx-auto">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-[10px] text-muted-foreground/60 uppercase tracking-widest font-bold">
              {totalActors.toLocaleString()} actors (min 3 movies) · {bankable} bankable (avg ≥500K)
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-3 gap-3">
        <div className="px-4 py-3 rounded-xl border border-border/30 bg-card">
          <p className="text-[9px] font-black uppercase tracking-widest text-muted-foreground/50">Total Actors</p>
          <p className="text-xl font-black">{totalActors.toLocaleString()}</p>
        </div>
        <div className="px-4 py-3 rounded-xl border border-border/30 bg-card">
          <p className="text-[9px] font-black uppercase tracking-widest text-muted-foreground/50">Bankable Stars</p>
