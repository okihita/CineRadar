...
function generateMockEngagementData() {
    ...
    const promoPerformance = promoTypes.map((promo) => ({
        promo_type: promo,
    ...
    const abTests = [
        { test_name: 'Flash Sale Timing', variant_a: '2 hours before', variant_b: '1 hour before', metric: 'Conversion Rate', result_a: '4.2%', result_b: '6.8%', winner: 'B', lift: '+62%' },
        { test_name: 'Discount Depth', variant_a: '20% off', variant_b: '30% off', metric: 'Revenue per Campaign', result_a: 'Rp85M', result_b: 'Rp78M', winner: 'A', lift: '+9%' },
        { test_name: 'CTA Button Color', variant_a: 'Blue', variant_b: 'Red', metric: 'Click Rate', result_a: '12.4%', result_b: '14.1%', winner: 'B', lift: '+14%' },
    ];
    ...
    return { promoPerformance, channelEffectiveness, loyaltyTiers, clvSegments, abTests, engagementTrend };
}

export async function GET() {
    return NextResponse.json(generateMockEngagementData());
}
