...
def get_branding_color(name: str) -> str:
    ...
    if any(x in n for x in ["IMAX", "MACRO XE", "ULTRA XD"]):
        return "#3b82f6"
    if any(x in n for x in ["PREMIERE", "GOLD", "VELVET", "VIP", "PLATINUM"]):
        return "#f59e0b"
    if "SWEET" in n:
        return "#ec4899"
    if "SATIN" in n:
    ...
    with suppress(Exception):
        ...
        for m in m_docs:
            ...
            for theatres in cities.values():
                t_entry = next(
                    (t for t in theatres if str(t.get("theatre_id")) == str(tid)), None
                )
                if t_entry:
                    break
                    ...
                    if st_doc.exists:
                        ...
                        if (
                            ...
                            if price is None:
                                ...
                                if pgs:
                                    price = pgs[0].get("seat_grd_price")
...
async def build_twin_v3_3_autonomous(
    ...
    if len(fp_counts) > 1:
        with open(COLLISION_LOG, "a") as f:
            f.write(
                f"THEATRE: {theatre_name} | STUDIO: {sid} | HARD COLLISION: {dict(fp_counts)}\n"
            )
        return 0
    ...
    if merchant.upper() == "XXI":
        ...
        for s in valid_samples:
            ...
            if p > price_map[cat]:
                price_map[cat] = p
        price_groups["01"] = {
            "name": primary_category.upper(),
            "color": get_branding_color(primary_category),
            "prices": price_map,
        }
    else:
        ...
        for g_id in all_grade_ids:
            ...
            price_groups[g_id] = {
                "name": g_name,
                "color": get_branding_color(g_name),
                "prices": p_map,
            }
    ...
    if merchant.upper() == "XXI":
        ...
        lane_indices = {lane["before_seat_column"] - 1 for lane in vertical_lanes}
        for row in seat_map:
            row_code = row.get("seat_code", "")
            new_row = {"row_name": row_code, "seats": []}
            for j, s in enumerate(row.get("seat_rows", [])):
                if j in lane_indices:
                    new_row["seats"].append({"id": "", "type": "aisle"})
                        ...
                        for r in sr.get("seat_map", []):
                            if r.get("seat_code") == row_code:
                                for ss in r.get("seat_rows", []):
                                    if (
                                        ss.get("seat_row") == label
                                        and ss.get("status") in [1, 5]
                                    ):
                                        real = True
                                        break
                        if real:
                            break
                new_row["seats"].append(
                    {
                        "id": label if real else "",
                        "type": "seat" if real else "aisle",
                        "grade": s.get("seat_grd_cd", "01") if real else None,
                    }
                )
                if real:
                    total += 1
                ...
                if s.get("seat_yn") == "1":
                    for sr in samples_data:
                        for ss in sr.get("seat_map", []):
                            if (
                                ss.get("row_name") == s.get("row_name")
                                and ss.get("seat_no") == s.get("seat_no")
                                and ss.get("seat_status") == 1
                            ):
                                real = True
                                break
                        if real:
    ...
    doc_ref = (
        db.collection(THEATRES).document(tid).collection("studios").document(sid)
    )
    with suppress(Exception):
        await doc_ref.update(
            {
                "is_ground_truth": firestore.DELETE_FIELD,
                "is_guessed": firestore.DELETE_FIELD,
                "verification_status": firestore.DELETE_FIELD,
                "last_updated": firestore.DELETE_FIELD,
                "physical_layout.version": firestore.DELETE_FIELD,
                "price_groups.01.price": firestore.DELETE_FIELD,
                "studio_id": firestore.DELETE_FIELD,
                "total_seats": firestore.DELETE_FIELD,
            }
        )
    ...
    await doc_ref.set(
        {
            "id": sid,
            "room_category": primary_category,
...
async def process_theatre_nitro(db: firestore.AsyncClient, theatre: Any) -> str:
    ...
    tasks = [
        build_twin_v3_3_autonomous(db, name, tid, sid, merchant) for sid in studio_ids
    ]
    ...
    if act_count > 0:
        ...
        await t_ref.set(
            {
                "total_capacity": t_cap,
                "studio_count": act_count,
                "version": 3.3,
