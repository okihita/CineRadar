...
async def main():
    ...
    for i, nearest in enumerate(valid_showtimes[:5]):
        ...
        layout = await scraper._fetch_seat_layout_api(nearest["showtime_id"], nearest["merchant"])
    ...
    print(
        f"   ✅ {occupancy['unavailable_seats']}/{occupancy['total_seats']} unavailable ({occupancy['occupancy_pct']}%)"
    )
