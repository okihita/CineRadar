...
Usage:
    python -m backend.scrapers movies [options]    # Scrape movie availability
    python -m backend.scrapers seats [options]     # Scrape seat occupancy
"""
import argparse
import asyncio
import json
...
def load_movie_data(data_dir: str = "data") -> Optional[Dict]:
...
def main():
    parser = argparse.ArgumentParser(
        ...
        epilog="""
Examples:
  python -m backend.scrapers movies --city JAKARTA
  python -m backend.scrapers movies --batch 0 --total-batches 9
  python -m backend.scrapers seats --mode morning
  python -m backend.scrapers seats --city JAKARTA --limit 10
        """
    )
