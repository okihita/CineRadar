...
interface PulseLeaderboardItem {
    rank: number;
    movie_id: string;
    title: string;
    tier: string;
...
export default function TikTokExplorerDatePage() {
    ...
    // Fetch movies with actual showtimes from schedules_v2
    ...
    // Check if the currently selected date matches the live crawl file
    const isDataAvailableForDate = selectedDate === crawlDate;

    // Daily crawl status: either live crawler data or Firestore pulse snapshot
    ...
    const handlePrevDay = () => {
        ...
        const prev = dt.toISOString().split('T')[0];
        router.push(`/tiktok/explorer/${prev}`);
    };
    ...
    const handleNextDay = () => {
        ...
        router.push(`/tiktok/explorer/${next}`);
    };
    ...
    // ─── 3. Actionable Market Signals (Live AI or Firestore Daily Pulse) ────────────
    ...
    const todayMovieSentimentList = useMemo(() => {
        ...
        // Start with active showtime movies
        const list = activeShowtimeMovies.map((m) => {
            const cleanTag = clean(m.title);
    ...
    // ─── 5. Filtered Posts & Comments Lists ──────────────────────────
    ...
    const filteredPulseLeaderboard = useMemo(() => {
        ...
        if (selectedMovieFilter !== 'all') {
            const cleanFilter = selectedMovieFilter.toLowerCase().replace(/[^a-z0-9]/g, '');
            list = list.filter((m) => {
    ...
    // Format date for header display (e.g. "11 Sep 2026")
    ...
    return (
        <div className="p-6 space-y-6 w-full">
                ...
                <div className="flex items-center gap-2 flex-wrap">
                    {selectedDate !== today && (
                        <Button
                            variant="outline"
                            size="sm"
                            onClick={() => router.push(`/tiktok/explorer/${today}`)}
                        ...
                        <input
                            ...
                            onChange={(e) => {
                                if (e.target.value && isValidDateFormat(e.target.value)) {
                                    router.push(`/tiktok/explorer/${e.target.value}`);
                                }
                            }}
            ...
            {isLiveLoading && !hasPulseData ? (
                ...
                /* Honest Empty State for Past Unrecorded Dates with no crawl nor pulse */
                <Card className="border-border/60 bg-card p-12 text-center space-y-4 max-w-lg mx-auto">
                    ...
                    <Button
                        variant="default"
                        size="sm"
                        onClick={() => router.push(`/tiktok/explorer/${crawlDate || today}`)}
                        className="gap-1.5 text-sm font-semibold rounded-lg"
                    >
                                ...
                                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
                                    ...
                                    <Card className="bg-gradient-to-br from-emerald-500/5 via-card to-card border-emerald-500/20">
                                        <CardHeader className="p-3.5 pb-1">
                                            ...
                                            <CardTitle className="text-base font-bold text-foreground truncate">
                                                {actionableInsights.womWinner.title || 'Audience Excitement'}
                                            </CardTitle>
                                        </CardHeader>
                                        <CardContent className="p-3.5 pt-1 space-y-1">
                                            <div className="flex items-baseline gap-2">
                                    ...
                                    <Card className="bg-gradient-to-br from-cyan-500/5 via-card to-card border-cyan-500/20">
                                        <CardHeader className="p-3.5 pb-1">
                                            ...
                                            <CardTitle className="text-base font-bold text-foreground truncate">
                                                {actionableInsights.viralityLeader.title || 'Daily Momentum'}
                                            </CardTitle>
                                        </CardHeader>
                                        <CardContent className="p-3.5 pt-1 space-y-1">
                                            <div className="flex items-baseline gap-2">
                                                <span className="text-2xl font-black font-mono text-cyan-600 dark:text-cyan-400">
                                                    {(actionableInsights.viralityLeader.shares || 0).toLocaleString()}
                                                </span>
                                                <span className="text-sm text-muted-foreground font-mono">
                                                    shares
                                                </span>
                                    ...
                                    <Card className="bg-gradient-to-br from-amber-500/5 via-card to-card border-amber-500/20">
                                        <CardHeader className="p-3.5 pb-1">
                                            ...
                                            <CardTitle className="text-base font-bold text-foreground truncate">
                                                {actionableInsights.frictionTarget.title || 'Showtime Availability'}
                                            </CardTitle>
                                        </CardHeader>
                                        <CardContent className="p-3.5 pt-1 space-y-1">
                                            <div className="flex items-baseline gap-2">
                                ...
                                <div className="flex items-center gap-2">
                                    ...
                                    >
                                        ...
                                        <Badge variant="secondary" className="text-sm font-normal px-1.5 py-0 h-5">
                                            {todayMovieSentimentList.length}
                                        </Badge>
                                    </button>
                        ...
                        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
                            <TabsList className="bg-muted/40 p-0.5 rounded-lg border border-border/40 h-auto">
                                <TabsTrigger value="videos" className="gap-2 text-sm font-semibold px-3 py-1.5 rounded-md">
                                    ...
                                    Viral Videos ({filteredPosts.length > 0 ? filteredPosts.length : filteredPulseLeaderboard.length})
                                </TabsTrigger>
                            ...
                            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                                <div className="flex items-center gap-3 flex-1">
                                    ...
                                    <span className="text-sm text-muted-foreground whitespace-nowrap">
                                        Showing <span className="font-mono font-medium">{filteredPosts.length > 0 ? filteredPosts.length : filteredPulseLeaderboard.length}</span> entries
                                    </span>
                                </div>
                                            ...
                                            <Card key={m.movie_id} className="p-4 bg-card border-border/50 flex flex-col justify-between space-y-3 hover:border-border transition-all">
                                                ...
                                                <div className="space-y-2 pt-2 border-t border-border/30 text-sm font-mono">
                                                    ...
                                                    <Button
                                                        variant="outline"
                                                        size="sm"
                                                        onClick={() => setModalMovie({ id: m.movie_id, title: m.title })}
                        ...
                        <TabsContent value="raw" className="space-y-3.5">
                            ...
                            <div className="bg-zinc-950 text-zinc-200 p-4 rounded-xl border border-border/40 overflow-x-auto max-h-[600px] font-mono text-sm leading-relaxed">
                                <pre>{JSON.stringify({
                                    date: selectedDate,
                                    hasPulseData,
                                    pulseData: pulseResponse?.data || null,
                                    actionableInsights,
                                    todayMovieSentimentList,
                                    totalPosts: filteredPosts.length,
                                    totalComments: filteredComments.length,
