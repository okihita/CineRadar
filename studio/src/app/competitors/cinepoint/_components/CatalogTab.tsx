...
import { useCatalogData, useSyncStream } from './useCatalogData';
import { DataTable, MovieCard, CatalogStatCard } from './CatalogViews';
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between flex-wrap gap-4">
        <div>
          <h3 className="text-lg font-bold text-foreground">Movie Database &amp; API Sync</h3>
          <p className="text-sm text-muted-foreground font-medium">
            Searchable catalog of verified Indonesian &amp; international titles synced from CinePoint.
          </p>
        </div>

        <div className="flex items-center gap-2">
          <Button variant="outline" size="sm" onClick={resetSync} disabled={syncing} className="h-8 gap-2 px-4 text-sm font-black uppercase tracking-wider rounded-xl border-border/60 hover:bg-muted transition-all">
            <RotateCcw className="w-3.5 h-3.5" /> Reset
          </Button>
          {syncing && (
            <Button variant="destructive" size="sm" onClick={stopSync} className="h-8 gap-2 px-4 text-sm font-black uppercase tracking-wider rounded-xl">Stop</Button>
          )}
          <Button variant={syncing ? 'secondary' : 'default'} size="sm"
            onClick={() => { if (!syncing) setShowTokenInput(!showTokenInput); }}
            disabled={syncing}
            className="h-8 gap-2 px-4 text-sm font-black uppercase tracking-wider rounded-xl transition-all">
      ...
      {showTokenInput && !syncing && (
        <div className="p-4 rounded-2xl border border-border/40 bg-card shadow-sm animate-in fade-in slide-in-from-top-2 duration-300">
          <p className="text-sm font-black uppercase tracking-widest text-muted-foreground/50 mb-3">Paste CinePoint Bearer Token</p>
          <div className="flex gap-2">
            <input type="text" value={syncToken} onChange={(e) => setSyncToken(e.target.value)} placeholder="eyJhbGciOiJIUzI1NiIs..."
              className="flex-1 px-3 py-2 rounded-lg border border-border/40 bg-muted/5 text-sm font-mono focus:outline-none focus:border-primary/40" />
            <Button size="sm" onClick={startSync} disabled={!syncToken.trim() || tokenExpired} className="h-9 gap-2 px-4 text-sm font-black uppercase tracking-wider rounded-xl">
              <Play className="w-3 h-3" /> Go
            </Button>
          </div>
          <div className="flex items-center gap-4 mt-2">
            <p className="text-sm text-muted-foreground/30">Copy from browser DevTools → Network → any /bff/ request → Authorization header</p>
            {tokenExpiryMin !== null && (
        ...
        <div className="p-4 rounded-2xl border border-border/40 bg-card shadow-sm space-y-3">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm font-black uppercase tracking-widest text-muted-foreground/60">Sync Progress</span>
            </div>
            {syncProgress && <span className="text-sm font-mono text-muted-foreground/50">{syncProgress.page}/{syncProgress.total} pages • {syncProgress.pct}%</span>}
          </div>
          {syncProgress && (
          ...
          {gatePending && (
            <div className="p-3 rounded-xl bg-amber-500/5 border border-amber-500/20 flex items-center justify-between">
              <span className="text-sm font-bold text-amber-600">Checkpoint: 3 pages validated. Ready to continue full backfill?</span>
              <div className="flex gap-2">
                <Button size="sm" variant="outline" onClick={stopSync} className="h-7 text-sm rounded-lg">Cancel</Button>
                <Button size="sm" onClick={startSync} className="h-7 text-sm rounded-lg">Continue All</Button>
              </div>
            </div>
          )}
          <div className="max-h-[120px] overflow-y-auto space-y-0.5">
            {syncLogs.map((log, i) => (<p key={i} className="text-sm font-mono text-muted-foreground/50">{log}</p>))}
          </div>
        </div>
      )}
        ...
        <div className="flex gap-1.5">
          {[null, 'local', 'international'].map((t) => (
            <button key={t ?? 'all'} onClick={() => { setTypeFilter(t); setPage(0); }}
              className={cn('px-3 py-1.5 rounded-lg border text-sm font-black uppercase tracking-widest transition-all',
      ...
      {data && !loading && (
        <div className="flex items-center gap-4 px-3 py-2 rounded-xl bg-muted/10 border border-border/20">
          ...
          <span className="text-sm font-mono text-primary/50 ml-auto">{data.pagination.total} rows → page {data.pagination.page + 1}/{data.pagination.total_pages}</span>
        </div>
      )}
      ...
      {loading ? (
          ...
          {data && data.pagination.total_pages > 1 && (
            <div className="flex items-center justify-center gap-2 pt-8">
              <Button variant="outline" size="sm" disabled={page === 0} onClick={() => setPage(page - 1)}
                className="h-8 px-4 text-sm font-black uppercase tracking-wider rounded-xl">Previous</Button>
              <span className="text-sm font-mono text-muted-foreground/50 px-3">Page {page + 1} / {data.pagination.total_pages}</span>
              <Button variant="outline" size="sm" disabled={page >= data.pagination.total_pages - 1} onClick={() => setPage(page + 1)}
                className="h-8 px-4 text-sm font-black uppercase tracking-wider rounded-xl">Next</Button>
            </div>
          )}
        </>
      ) : (
        <div className="py-16 text-center border-2 border-dashed rounded-[2.5rem] border-border/40 bg-muted/5">
          ...
          <p className="text-muted-foreground text-sm font-bold uppercase tracking-widest">No movies in catalog yet.</p>
          <p className="text-muted-foreground/50 text-sm mt-2 uppercase tracking-tight font-medium">Click &quot;Start Sync&quot; above to begin scraping from CinePoint</p>
        </div>
      )}
    </div>
  );
