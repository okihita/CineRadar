  ...
  return (
    <Card>
      ...
      <CardContent className="space-y-4">
        {directors.length > 0 && (
          <div>
            <p className="text-sm font-black uppercase tracking-widest text-muted-foreground/60 flex items-center gap-1 mb-1">
              <Megaphone className="w-3 h-3" /> Director{directors.length > 1 ? 's' : ''}
            </p>
            <div className="flex flex-wrap gap-2">
              {directors.map((name) => (
                <Badge key={name} variant="outline" className="border-amber-500/20 text-amber-700 text-sm">{name}</Badge>
              ))}
            </div>
          </div>
        )}
        {writers.length > 0 && (
          <div>
            ...
            <div className="flex flex-wrap gap-2">
              {writers.map((name) => (
                <Badge key={name} variant="outline" className="border-indigo-500/20 text-indigo-700 text-sm">{name}</Badge>
              ))}
            </div>
          </div>
        )}
        {casts.length > 0 && (
          <div>
            ...
            <div className="flex flex-wrap gap-1.5">
              {casts.map((name) => (
                <span key={name} className="text-sm text-muted-foreground bg-muted/50 px-2 py-0.5 rounded-md">{name}</span>
              ))}
            </div>
          </div>
        )}
        ...
        <div className="space-y-1">
          {userRatings.map((r) => (
            <div key={r.rating} className="flex items-center gap-2">
              <span className="text-sm font-mono w-4 text-right text-muted-foreground/60">{r.rating}</span>
              ...
              <span className="text-sm font-mono w-10 text-right text-muted-foreground">{r.value}%</span>
            </div>
          ))}
        </div>
      </CardContent>
