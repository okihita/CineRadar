    ...
    if (error || !data) {
        return (
            <div className="flex flex-col items-center justify-center py-20 gap-4 border border-dashed rounded-xl bg-red-500/5">
                ...
                <p className="text-sm text-muted-foreground">{error?.message || 'No data available'}</p>
            </div>
        );
    }
        ...
        <div className="space-y-6">
                                ...
                                <div key={stat.merchant} className="space-y-2">
                                    <div className="flex justify-between items-end">
                                        <div>
                                            ...
                                            <span className="ml-2 text-sm text-muted-foreground uppercase font-bold opacity-50">
                                                {stat.totalStudios} Studios
                                            </span>
                                        </div>
                                        <div className="text-right">
                                            <span className={stat.collisionRate === 0 ? "text-green-500 font-bold text-sm" : "text-amber-500 font-bold text-sm"}>
                    ...
                    <CardContent className="p-6 flex flex-col justify-between h-[300px]">
                        <div className="grid grid-cols-2 gap-8">
                            <div className="space-y-4">
                                <div className="flex items-center gap-3">
                                    ...
                                    <div>
                                        ...
                                        <p className="text-sm font-bold uppercase text-muted-foreground">ATMOS Units</p>
                                    </div>
                                </div>
                                <div className="flex items-center gap-3">
                                    ...
                                    <div>
                                        ...
                                        <p className="text-sm font-bold uppercase text-muted-foreground">3D Units</p>
                                    </div>
                                </div>
                            </div>
                            <div className="flex flex-col justify-center items-center border-l border-dashed pl-8">
                                ...
                                <p className="text-sm font-black uppercase tracking-[0.2em] text-center mt-2 text-muted-foreground/60">
                        ...
                        {data.regionalPricing.map((price, idx) => (
                            <div key={price.city} className="flex-shrink-0 w-32 space-y-2 group">
                                <div className="h-32 bg-muted/20 rounded-t-lg relative flex items-end overflow-hidden border border-border/30">
                                    ...
                                    />
                                    <div className="absolute top-2 left-2 text-sm font-black text-foreground">#{idx + 1}</div>
                                </div>
                                <div className="px-1 text-center">
                                    <p className="text-sm font-black uppercase truncate">{price.city}</p>
                                    <p className="text-sm font-bold text-green-600">Rp {price.avgPrice.toLocaleString('id-ID')}</p>
                                </div>
                            </div>
                        ))}
                    </div>
                    <div className="p-3 bg-green-500/5 rounded-lg border border-green-500/10">
                        <p className="text-sm text-muted-foreground leading-relaxed uppercase font-bold tracking-tight text-center">
            ...
            <Card className="border-red-500/10 bg-red-500/[0.02]">
                <CardHeader className="pb-2 border-b border-red-500/10 bg-red-500/[0.05]">
                    <CardTitle className="text-sm font-black uppercase tracking-[0.2em] text-red-600 flex items-center gap-2">
                ...
                <CardContent className="p-0">
                    <div className="max-h-[400px] overflow-y-auto">
                        <table className="w-full text-left border-collapse">
                            <thead className="bg-muted/30 sticky top-0 z-10">
                                <tr>
                                    <th className="p-3 text-sm font-black uppercase tracking-widest text-muted-foreground border-b border-border/50">Status</th>
                                    <th className="p-3 text-sm font-black uppercase tracking-widest text-muted-foreground border-b border-border/50 text-right">Detection</th>
                                </tr>
                            </thead>
                            <tbody>
                                <tr className="border-b border-border/30 hover:bg-muted/10 transition-colors">
                                    <td className="p-4">
                                        <div className="flex items-center gap-2">
                                            ...
                                            <div>
                                                <p className="text-sm font-black uppercase tracking-tight">Atomic Integrity Rate</p>
                                                <p className="text-sm text-muted-foreground font-medium uppercase tracking-tighter">Percentage of rooms with stable fingerprints</p>
                                            </div>
                                        </div>
                                    </td>
                                    <td className="p-4 text-right">
                                        <Badge variant="outline" className="bg-green-500/10 text-green-600 border-green-500/30 text-sm font-black">
                                            98.7% SECURE
                                        </Badge>
                                    </td>
                                </tr>
                                <tr className="border-b border-border/30 hover:bg-muted/10 transition-colors">
                                    <td className="p-4">
                                        <div className="flex items-center gap-2">
                                            ...
                                            <div>
                                                <p className="text-sm font-black uppercase tracking-tight">Identity Drift Rate</p>
                                                <p className="text-sm text-muted-foreground font-medium uppercase tracking-tighter">Percentage of rooms with multi-category collisions</p>
                                            </div>
                                        </div>
                                    </td>
                                    <td className="p-4 text-right">
                                        <Badge variant="outline" className="bg-amber-500/10 text-amber-600 border-amber-500/30 text-sm font-black">
                                            1.3% QUARANTINE
                                        </Badge>
                                    </td>
                                </tr>
                    ...
                    <div className="p-4 border-t border-red-500/10 text-center">
                        <p className="text-sm text-muted-foreground font-bold uppercase tracking-[0.1em]">
